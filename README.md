...
**Coming up.** Day notes and deadlines are the two things on a calendar that leave almost no mark on the month grid: neither is a category-dot category, and the edge bar above is drawn only for the items a block owns. Everything else — another page's deadline, anything filed in the Schedule — used to be reachable only by clicking the day it happened to sit on. A **Coming up** list at the foot of every calendar block closes that: every day note and deadline dated **on or after today**, in date-then-time-then-title order, with no click at all.

It is deliberately **not** tied to the month in the grid above it. Paging ‹ › re-draws the grid and leaves the list alone, because what a document's calendar is asked for is what is still ahead. Each row carries its date, a `📌` or `⏰`, the time if the item has one, the title, and — for a deadline — how many caution days it has, the count rather than one row per day, so a three-week run-up stays one line. A ticked deadline stays listed and is drawn struck through in green: the tick suppresses the `!` marks, it has never removed the deadline. The first eight rows show, with `+ N more` for the rest; the list is capped for length, never filtered. When nothing is ahead it says so and **names the cutoff** — "Nothing on or after 2026-09-20" — so an item filed yesterday does not read as lost.

The list obeys the filter bar, so it and the grid can never disagree about what the chips just switched off. It does **not** obey the `⌾` scope: like the day panel it shows every origin, with the same `📄 <page title>` chip and the same highlight on the rows this block owns — an item another page put on the calendar is exactly the item that was hardest to find. Every row is read-only and leads to its own day, moving the grid's month when it needs to; the edit controls stay in the day panel, where the ownership rule already decides who may press them. Nothing about the list is stored: it is always on, and how far it has been expanded is not something the document remembers. It prints, unlike the block's controls, so an exported PDF no longer carries a month grid with nothing saying what is on it.
